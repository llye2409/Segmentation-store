...
def seg_app():
    ...
    mode = st.radio("Chọn phương thức phân loại khách hàng", ("📊 Dự đoán theo chỉ số RFM", "🧮 Phân tích từ dữ liệu giao dịch"))

    if mode == "📊 Dự đoán theo chỉ số RFM":
        with st.form("rfm_input_form"):
            ...
            with st.expander("📘 Giải thích các chỉ số", expanded=False):
                st.markdown("""
                - **Recency (🕒)**: Số ngày kể từ lần mua gần nhất. Nhập giá trị từ 0 - 700.
                - **Frequency (🔁)**: Số lần mua hàng. Nhập giá trị từ 1 đến 35.
                - **Monetary (💰)**: Tổng số tiền đã chi tiêu. Nhập giá trị từ 1 đến 360.0
                """)

            col1, col2, col3 = st.columns(3)
            recency = col1.number_input("🕒 Recency (ngày)", min_value=0, step=1, max_value=750)
            frequency = col2.number_input("🔁 Frequency (lần)", min_value=1, step=1, max_value=35)
            monetary = col3.number_input("💰 Monetary (USD)", min_value=1.0, max_value=360.0, format="%.2f")

            submitted = st.form_submit_button("🚀 Phân loại khách hàng")
        ...
        uploaded_file = st.file_uploader("📁 Tải lên file dữ liệu giao dịch (.csv)", type=["csv"])
        ...
        if uploaded_file is not None:
            try:
                ...
                if not expected_columns.issubset(df.columns):
                    ...
                    # Tiến hành phân tích phân khúc khách hàng
                    segmentor = CustomerSegmentation(model, scaler, r_bins, f_bins, m_bins)
                    visualizer = SegmentationVisualizer()
                    
                    # Tiến hành xử lý và tính toán RFM
                    rfm_df = segmentor.segment_customers(df)
                    
                    if rfm_df is not None and not rfm_df.empty:
                        # 👉 Kiểm tra cảnh báo về giá trị RFM nếu có
                        rfm_warnings = segmentor.check_rfm_range(rfm_df)
                        if rfm_warnings:
                            for warning in rfm_warnings:
                                st.warning(warning)

                        # Hiển thị thông tin phân tích phân khúc khách hàng
                        ...
                        st.warning("⚠️ Không có dữ liệu phân khúc khách hàng hợp lệ.")
