...
class CustomerSegmentation:
    def __init__(self, model, scaler, r_bins, f_bins, m_bins, cluster_map=None):
        self.model = model
        self.scaler = scaler
        self.r_bins = r_bins
        self.f_bins = f_bins
        self.m_bins = m_bins
        self.rfm_stats = {
            'Recency': {'min': 0, 'max': 700},
            'Frequency': {'min': 1, 'max': 35},
            'Monetary': {'min': 1.0, 'max': 360.0}
        }
    ...
    def check_rfm_range(self, rfm_df):
        for col in ['Recency', 'Frequency', 'Monetary']:
            min_val = self.rfm_stats[col]['min']
            max_val = self.rfm_stats[col]['max']
            if rfm_df[col].min() < min_val or rfm_df[col].max() > max_val:
                return ["⚠️ Một số giá trị RFM nằm ngoài phạm vi huấn luyện, kết quả có thể sai lệch."]
        return []


    def prepare_rfm_features(self, rfm_df):
        try:
            df_copy = rfm_df.copy()

            # Cảnh báo nếu dữ liệu bị cắt
            for col, bins in zip(['Recency', 'Frequency', 'Monetary'],
                                 [self.r_bins, self.f_bins, self.m_bins]):
                before_clip = df_copy[col].copy()
                df_copy[col] = df_copy[col].clip(lower=bins[0], upper=bins[-1])
                if not before_clip.equals(df_copy[col]):
                    pass
                    #st.warning(f"Một số giá trị của {col} đã được giới hạn trong khoảng bin")

            df_copy['R'] = pd.cut(df_copy['Recency'], bins=self.r_bins,
                                  labels=[4, 3, 2, 1], include_lowest=True).astype(int)
            df_copy['F'] = pd.cut(df_copy['Frequency'], bins=self.f_bins,
                                  labels=[1, 2, 3, 4], include_lowest=True).astype(int)
            df_copy['M'] = pd.cut(df_copy['Monetary'], bins=self.m_bins,
                                  labels=[1, 2, 3, 4], include_lowest=True).astype(int)
    ...
    def segment_customers(self, df):
        try:
            ...
            X_RFM = self.prepare_rfm_features(rfm_df)
            if X_RFM.empty:
                return pd.DataFrame()
            ...
            rfm_df['Segment'] = rfm_df['Cluster'].map(self.cluster_map)
